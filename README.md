...
自动生成结构化的 Excel 数据分析报告。上传数据文件 → AI 聊天对话确认需求（或选择模版）→ 自动生成 PDF 报告 → 下载/打印。支持多文件合并/分别分析、模版复用、历史管理。
...
- 多文件上传（.xlsx/.xls/.csv，单文件 ≤100MB，最多 20 个）
- AI 聊天确认分析需求（DeepSeek V4 + 规则引擎混合模式）
- 分析模版保存与复用（双模式：模版模式/对话模式）
- 结构化 PDF 报告生成（数据概览/描述性统计/图表/交叉分析）
- 报告下载（支持断点续传）+ 浏览器打印
