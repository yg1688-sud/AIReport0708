...
public class LlmClient
{
    ...
    private static string BuildSystemPrompt(string[] availableColumns)
    {
        return "你是一个专业的数据分析助手，帮助用户分析 Excel 数据。\n\n" +
            ...
            "6. 回复中严禁使用 Markdown 表格语法（即用 | 分隔的表格），也不要展示示例数值或示例命名（如\"例如\"、\"命名为\"等）。确认需求时直接陈述计算规则，不附加举例说明。\n" +
            "7. 总结确认需求时，表格列必须用无序列表逐行列出，格式严格为\"- **列名**：计算逻辑\"，列名使用可用数据列中的原始名称（如\"员工ID\"而非\"员工\"），计算逻辑简洁说明即可。禁止在列表项中加入额外描述、小节标题或分组标签。\n" +
            "8. 如需列出多组枚举值（如ERPID分组），每组必须独占一行无序列表，格式为\"- 值1和值2（合并为一组）\"或\"- 值1\"。必须列出每一个具体的数值，严禁用\"其余\"\"其他\"\"等\"等词省略任何一组。\n" +
            "10. 如需生成图表（柱状图/折线图/饼图/漏斗图/条形图），必须在确认需求中明确写出：横轴为XX，纵轴为YY。横轴和纵轴的列名必须使用可用数据列中的原始名称。\n\n" +
            "9. 确认需求时，在【需求已明确，请输入确认生成报告】之前，紧邻附加一行机器可读标记（用户不可见，仅用于系统解析），格式如下：\n" +
            "<!--PARAMS group=分组列名 sum=求和列名 pivot=透视列名 values=值1;值2+值3;值4-->\n" +
            "其中：group填分组所依据的列名，sum填求和的列名（如是去重计数则填dedup=去重列名），pivot填透视列名（如无则省略），values填枚举值（分号分隔不同组，加号连接同组合并值，如无则省略）。所有列名必须使用可用数据列中的原始名称。\n\n" +
